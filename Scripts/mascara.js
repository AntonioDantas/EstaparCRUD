...
function formataTelSemDDD(campo, evt) {
    //0000-0000
    var xPos = PosicaoCursor(campo);
    evt = getKeyCode(evt);
    var tecla = getEvent(evt);
...
function formataAcento(campo, evt, rigoroso) {
    evt = getEvent(evt);
    xPos = PosicaoCursor(campo);
    var tecla = getKeyCode(evt);
    if (!teclaValida(tecla))
    ...
    var i;
    var valor = campo.value = filtraCampo(campo);
    ...
    var validos = 'qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890 ';
    for (i = 0; i < valor.length; i++) {
        if (caracteresInvalidos.indexOf(valor[i]) != -1) {
            nova = caracteresValidos.charAt(caracteresInvalidos.indexOf(valor[i]));
            valor = valor.substring(0, i) + nova + valor.substring(i + 1);
            i = 0;
        }
        else if (rigoroso == 1) {
            if (validos.indexOf(valor[i]) == -1) {
